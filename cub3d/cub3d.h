...
/*   Updated: 2020/12/21 13:16:24 by aiglesia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void *mlx_ptr);
int					fill_resolution(char *line, t_render_data *render_data,
t_error *error);
...
int					check_map_basic_elements(t_map_bearings *map_info,
					t_error *error);
...
void				load_floor_ceiling(t_render_data *render_data,
					t_mlx *mlx_data);
...
int					initialize_render_data(t_mlx *mlx_data, t_cub3d *data,
					char *file_path);
...
float				calculate_collision(float angle, t_cub3d *data,
					float x, float y);
int					calculate_collision_0(float p_x, float p_y,
					t_ray_tracing *ray_trc, char **map);
int					calculate_collision_1(float p_x, float p_y,
					t_ray_tracing *ray_trc, char **map);
int					calculate_collision_2(float p_x, float p_y,
					t_ray_tracing *ray_trc, char **map);
int					calculate_collision_3(float p_x, float p_y,
					t_ray_tracing *ray_trc, char **map);
...
unsigned int		add_shade(unsigned int colour, double distance,
					float shade_distance);
void				draw_column(int i, float distance, t_cub3d *data,
					int y_offset);
void				move_from_wall(t_cub3d *data, float x, float y);
t_bool				check_wall_distance(t_cub3d *data, float temp_x,
					float temp_y);
t_bool				check_sprite(t_cub3d *data);
float				get_sprite_horizontal_lenght(t_ray_tracing *ray_trc,
					float view_angle);
...
void				render_health_bar(t_mlx *mlx_data, float health_ratio,
					int pixel_size, t_keys *keys);
int					handle_transition(t_cub3d *data);
void				order_sprites(t_list *sprite);
void				draw_sprites(t_cub3d *data, float
					*distance_array, float time);
void				draw_floor_and_ceiling(t_cub3d *data,
					int y_offset, float *distance_array);
void				draw_walls(t_cub3d *data, float
					*distance_array, float *offset_array,
					int *dir_array);
