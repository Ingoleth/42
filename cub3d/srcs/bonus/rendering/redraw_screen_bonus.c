...
void	print_debug_info(t_mlx *mlx_data, t_render_data *render_data)
{
	char *s1;
	char *s2;
	char *s3;
...
void	draw_face(t_health_bar *health_bar, t_keys *keys)
{
	...
	while (i < final_pos)
	{
		while (j < final_pos)
		{
			draw_pixel(health_bar->image, i, j, get_pixel(health_bar->face,
			index + i - health_bar->face_pos, j - health_bar->face_pos));
			j++;
		}
		i++;
		j = health_bar->face_pos;
...
void	render_health_bar(t_mlx *mlx_data, float health_ratio, int pixel_size,
t_keys *keys)
{
	int start_pos_x;
	int start_pos_y;
	...
	draw_pixel_area(mlx_data->health_bar.image, set_draw_coords(start_pos_x,
	start_pos_y, end_pos_x, end_pos_y), red_1);
	draw_pixel_area(mlx_data->health_bar.image, set_draw_coords(start_pos_x,
	start_pos_y, end_pos_x, start_pos_y + pixel_size), red_2);
	draw_pixel_area(mlx_data->health_bar.image, set_draw_coords(start_pos_x,
	end_pos_y - pixel_size, end_pos_x, end_pos_y), red_2);
	if (end_pos_x - start_pos_x > pixel_size)
		draw_pixel_area(mlx_data->health_bar.image, set_draw_coords(end_pos_x -
		pixel_size, start_pos_y + pixel_size, end_pos_x, end_pos_y -
		pixel_size), red_2);
	draw_pixel_area(mlx_data->health_bar.image, set_draw_coords(end_pos_x,
	start_pos_y, start_pos_x + mlx_data->health_bar.bar_lenght_x, end_pos_y),
	BLACK);
	mlx_put_image_to_window(mlx_data->mlx_ptr, mlx_data->win_ptr,
	mlx_data->health_bar.image->img, mlx_data->map->width + 2 * pixel_size, 0);
...
int		redraw_screen(t_cub3d *data)
{
	...
	{
		mlx_put_image_to_window(mlx_data->mlx_ptr, mlx_data->win_ptr,
		mlx_data->map->img, 0, 0);
		render_cursor(mlx_data, &data->render_data);
		print_debug_info(mlx_data, &data->render_data);
		render_health_bar(mlx_data, data->render_data.current_health /
		(float)MAX_HEALTH, mlx_data->health_bar.pixel_size,
